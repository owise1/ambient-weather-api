...
var API_URL = 'http://api.ambientweather.net/';
...
module.exports = function (_EventEmitter) {
  ...
  _createClass(AmbientWeatherApi, [{
    ...
    value: function connect() {
      ...
      this.socket.on('connect', function (what) {
        console.log('connected');
        _this3.socket.emit('subscribe', { apiKeys: [_this3.apiKey] });
      });
      this.socket.on('error', function (err) {
        console.log('error');
        console.log(err);
      });
      this.socket.on('subscribed', function (res) {
